...
public:
	...
	// Devuelve true si el punto nuevo es más pequeño
	bool moreLeftDown(const CGPoint& p) const {
		if (m_X != p.m_X)
			return m_X < p.m_X; 
		else
			return m_Y < p.m_Y;
	...
	// Devuelve true si el punto nuevo es más pequeño
	bool moreRightUp(const CGPoint& p) const {
		if (m_X != p.m_X)
			return m_X > p.m_X;
		else
			return m_Y > p.m_Y;
...
std::list<CVertex*> QuickHullRecursive(CVertex* A, CVertex* B, std::list<CVertex*> punts);
