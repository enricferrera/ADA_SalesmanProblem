...
int tram = -1; // En la primera iteraci�n siempre pasa a 0
...
void SalesmanTrackBacktrackingRecursiu(NodeCami* pAnterior, NodeCami* node, CVisits& visits)
{
	...
	// Si hem arribat al dest�
		// Hem passat per tots els vertexs
			// --> es solucio
		// Si visitas completadas y camino m�s corto tenemos nueva solucion
		// --> guardas solucions
...
void SalesmanTrackBacktrackingRecursiu(NodeCami* pAnterior, CVertex* pActual, CVisits& visits)
{
	...
	if (pActual == pDesti) {
		...
		pActual->m_visitesVisitades.back() = true; // Posem dest� com visitat
		...
		// Si totes les visites i cami m�s curt
	...
	if (LongitudCamiActual < LongitudCamiMesCurt) {
		// Si no hem passat en general i es visita, cambiem de tram i marquem que aquest node comen�a aquest tram
		...
		// Mirem tots els edges y mirem si el dest� es una opci�
		...
		// Sin� es una visita, lo quitas del tramo y ya
	...
	// Paso adelante
		// A�adir al stack, marcar como visto y a�adir al tramo
		// �Para a�adir con que no sea parte del tramo ya vale?
	...
	/*
		Major problema, al comprobar si un vertice es apto para continuar, hemos de ver si en ese tramo a�n no hemos pasado. C�mo lo hacemos? Miramos en la pila hasta la �ltima visita que hab�amos visitado por primera vez. Y si paso por un nodo visita que no marca el comienzo del tramo.
		Como cojones hago eso?
			- Recorrer hac�a atras los vertices hasta encontrar uno que sea visita
			- Variable en Vertice que indique que tramo marca esa visita. Ej: Si ese vertice visita marca el comienzo del tramo 2, y estamos en el 3, al recorrer el camino hac�a atras, lo haremos hasta el vertice que tenga el indicador de iniciador de tramo en 3.
	...
	*/
	...
	// Usar� la structura nodo para reacer el camino
	// Y usar� la lista de vertices para comprobar tramos y visitas
//}
...
{
	// Para cada visita, mira el camino m�s corto de todas las visitas a la actual
	size_t i = 0;
	for (auto visit : visits.m_Vertices) {
		...
		for (auto visitDijkstra : visits.m_Vertices) {
			...
			while (vertexActual != visit && vertexActual->m_pDijkstraPrevious != nullptr) {
				...
				vertexActual = vertexActual->m_pDijkstraPrevious->m_pOrigin; // Es origin->recorre al rev�s
			}
			j++;
		}
		i++;
